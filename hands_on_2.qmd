...
XXXX @Sandra gibt es hier ein gutes Kapitel das all das schön zusammenfasst?
...
Wir haben in diesen Übungen bereits einige Funktionen verwendet (z.b `sum()` oder `sort()` ) Funktionen bestehen aus einem Namen und Argumenten. In diesem Beispiel wird die Funktion "`sort()` " aufgerufen, die Argumente die dafür benötigt werden sind `first_vector` & `decreasing = TRUE` . Die meisten Funktionen haben "defaults" also argumente die eine voreinstellung haben. Wenn man diese nicht verändern möchte müssen diese auch nicht definiert werden. `sort()` hat als default `sdecreasing = FALSE` , deswegen müssen wir um die höchsten Werte des Vektors zu bestimmen diesen Default umstellen.

Die Defaults der Funktionen lassen sich via Hilfefunktion nachschauen. Diese Kann entweder auf dem "Help" Tab geöffnet werden oder direkt via Konsole `?Funktionsname`
...
Suche mit der Hilfefunktion nach dem default von `?sum()` . Korrigiere den folgenden code so dass die Summe des Vektors berechet wird.

```{r}
vector_with_nas <- c(1 ,2,3,5,6,NA, 14, 21, NA, 12)
sum(vector_with_nas)
```
